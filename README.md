...
팔로우한 치지직 스트리머가 방송을 켜면 알림을 보내주는 Windows 프로그램.
...
설정 패널의 **"부팅 시 자동 실행"** 체크박스를 켜면 됨 (레지스트리 HKCU Run에 등록).
exe로 실행 중이면 exe가, 스크립트로 실행 중이면 `pythonw app.py`가 등록된다.
자동 실행 시에는 `--minimized` 옵션으로 창 없이 트레이에서 조용히 시작한다.
수동 실행에서도 트레이로만 시작하려면 `--minimized`를 붙여 실행하면 됨.
